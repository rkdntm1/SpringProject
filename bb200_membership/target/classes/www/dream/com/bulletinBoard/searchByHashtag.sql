...
select id
  from s_hashtag
 where hashtag in ('기본', '처리')

-- 검색어 "기본 처리"로 단어가 적용된 게시글 id를 찾으시오
...
select distinct m.post_id
 ...
 where w.hashtag in ('기본', '처리') 
   and w.id = m.hashtag_id
  

-- 검색어 "기본 처리"로 단어가 적용된 게시글과 작성자 정보를 찾으시오.
select distinct p.*, w.*
  from s_hashtag h, sm_ht2post m, s_post p, s_party w
 where h.hashtag in ('기본', '처리')
   and h.id = m.hashtag_id
   and m.post_id = p.id
   and p.writer_id = w.user_id

-- 검색어 "기본 처리"로 단어가 적용된 게시글과 작성자 정보를 찾아서
...
-- 단어 'element'를 s_hashtag에서 검색을 하면 그 결과는 {'element', 'feature', '객체', '지향'}이다.
SELECT hashtag, id
   FROM s_hashtag
...
-- 검색어 'element 기본 처리'로 이 단어가 계층적으로 적용된 게시글과 작성자 정보를 찾아서
-- 해당 페이지의 내용을 최신 작성글 순서로 출력하시오.
select distinct p.*, w.*
 ...
 where h.id in (
 	   SELECT id
   	     FROM s_hashtag
        start with hashtag in ('element', '기본', '처리') 
      CONNECT BY PRIOR id = super_id
 		)
   and h.id = m.hashtag_id
   and m.post_id = p.id
