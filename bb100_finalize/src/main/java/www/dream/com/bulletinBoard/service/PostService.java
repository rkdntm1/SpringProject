...
public class PostService {
	...
	public List<PostVO> getListByHashTag(Party curUser, int boardId, Criteria cri) {
		if (cri.hasSearching()) { // 검색이 있을때
			...
			if (curUser != null) {
				mngPersonalFavorite(curUser.getUserId(), searchingHashtags);
			}
	...
	private void createHashTagAndMapping(Class targetType, String hashMapOpponent, Map<String, Integer> mapOccur, Set<String> setHashTag) {
		...
		if (targetType == PostVO.class) {
			hashTagMapper.insertMapBetweenPost(setExisting, hashMapOpponent);	//기존 단어 및 신규 단어 와 짝짓기	
		} else if (targetType == Party.class) {
			hashTagMapper.insertMapBetweenParty(setExisting, hashMapOpponent);	
		}
	}
	...
	private void mngPersonalFavorite(String userId, String[] searchingHashtags) {
		//기존 단어와 신규 단어로 분할
		...
		//기존 단어는 활용 횟수 올리기
		//신규 단어는 단어 등록 및 활용 횟수는 1로 설정
		hashTagMapper.recordPersonalFavorite();
	}
}
