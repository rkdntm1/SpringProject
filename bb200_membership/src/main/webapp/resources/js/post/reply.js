...
var replyService = (function() {
	...
	function CheckPartyId(partyId, successCallBack, errorCallBack) {
		$.getJSON(
				"/party/idChecker/" + partyId,
				function(cnt) {
					if (successCallBack) {
						successCallBack(cnt);
					}
	...
	function getListOfReply(replyId, successCallBack, errorCallBack) {
		$.getJSON(
			...
			function(listAllReplyOfReply) { //controller에서 리턴받는거
				if (successCallBack) { 
					successCallBack(listAllReplyOfReply); // jsp에서 만든 callback함수 
				}
			}
		).fail(
			function(xhr, status, errMsg){
